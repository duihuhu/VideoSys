...
def all_to_all_with_pad(
    ...
    print(f"before all to all: {input_.size()}. sp: {torch.distributed.get_rank(process_group)}. sp size: {torch.distributed.get_world_size(process_group)},  global_rank: {torch.distributed.get_rank()}. scatter_dim: {scatter_dim}, gather_dim: {gather_dim}")
    
    input_ = _AllToAll.apply(input_, process_group, scatter_dim, gather_dim)
    print(f"after all to all: {input_.size()}. sp: {torch.distributed.get_rank(process_group)}. sp size: {torch.distributed.get_world_size(process_group)}, global_rank: {torch.distributed.get_rank()}. scatter_dim: {scatter_dim}, gather_dim: {gather_dim}")
